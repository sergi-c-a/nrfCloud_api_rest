...
app.get('/ubicacion', async (req, res) => {
    try {
        ...
        // Buscar el mensaje que contiene la ubicación GNSS (GPS)
        let locationData = {};
        if (data.items) {
            data.items.forEach(item => {
                const message = item.message;
                // Cambia "GROUND_FIX" por "GNSS" para obtener la posición GPS real
                if (
                    message.appId === "GNSS" &&
                    message.data &&
        ...
        console.log("📌 Coordenadas GNSS del dispositivo:", locationData);
